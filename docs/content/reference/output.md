...
describes what users get. Pick a format with `-o`, or let jsinfo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jsinfo <command> -o table   # aligned columns for reading
jsinfo <command> -o jsonl   # one JSON object per line, for piping
jsinfo <command> -o json    # a single JSON array
jsinfo <command> -o csv     # spreadsheet friendly
jsinfo <command> -o tsv     # tab-separated
jsinfo <command> -o url     # just the URL column
jsinfo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jsinfo <command> --fields id,title,url
```
...
```bash
jsinfo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jsinfo <command>            # a table, because this is a terminal
jsinfo <command> | wc -l    # JSONL, because this is a pipe
```
